...
- (UIImage *)gradientImage {
    CGFloat colors[] = {0.0, 1.0, 0.0, 1.0, 1.0, 1.0};
	...
	CGImageRef imageWithMask = CGImageCreateWithMask(ret.CGImage, mask);
...
{
    for (UIView *temp in self.subviews) {
		if ([temp isKindOfClass:[UIImageView class]] && temp.frame.origin.y==self.frame.size.height+REFLECTION_MARGIN) {
...
- (void)addReflectionEffect {
	...
	frame.origin = CGPointMake(0.0f, self.frame.size.height+REFLECTION_MARGIN);
